...
const NavBtn = (props: TMainButton) => {
  ...
  return (
    <button
      className={` cursor-pointer min-w-[240px] h-[auto]  text-SH origin-[0%_50%] bg-transparent group after:block after:scale-0 after:scale-x-100 after:border-[2px] ${
    ...
    >
      <div className={`flex w-auto h-auto justify-between items-center p-1`}>
        <span
          ...
          } group-after:block  group-after:transition-transform group-after:duration-[500ms]  group-hover:after:scale-100`}
        >
          {text}
        </span>
        {length ? (
          <div
            ...
            } min-w-[36px] min-h-[36px] rounded-full text-center  group-after:transition-transform group-after:duration-[250ms] group-hover:text-white group-hover:after:scale-100`}
          >
            {length}
          </div>
        ) : null}
